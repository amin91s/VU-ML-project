...
### this definition is probably not needed anymore
def create_dataset(img_folder): 
    img_data_array = []  ## all images list
    for img in os.listdir(img_folder):
              ## selecting image from the directory
        imagepath = os.path.join(img_folder, img)
        image=cv2.imread(imagepath,cv2.COLOR_BGR2RGB) ## reading the image 
        image=cv2.resize(image,(100,100))
        img_data_array.append(image)   
    return img_data_array
img_data = create_dataset(r'..\ML\dataverse_files\HAM10000_images')
df = df.assign(img=img_data)
...
#data exploration type skincancer
plt.figure(figsize=(10,5))
plt.title("Type skincancer",fontsize=15)
plt.ylabel("Count")
df['diagnosis'].value_counts().plot(kind='bar')

#localization of skincancer, age & sex distribution
plt.figure(figsize=(10,5))
...
plt.ylabel("count")
df['age'].hist(bins=40, figsize=(10,5), title="Age")
...
plt.subplot(131)
plt.title('Age vs. Diagnosis',fontsize = 15)
sns.countplot(y='age', hue='diagnosis',data=df)
plt.subplot(132)
plt.title('GENDER VS CELL TYPE',fontsize = 15)
sns.countplot(y='sex', hue='diagnosis',data=df)
